...
export function ListEnigmaTable ({ adventure}: Props){
    ...
    const loadEnigmas = useCallback(async () => {
      ...
      const msg =
        e instanceof Error
          ? e.message
          : "Erreur lors du chargement des enigmas.";
      setError(msg);
      toast.error(msg);
      setEnigmas([]);
      setTotal(null);
...
const totalPages =
   ...
   const showInitialSkeleton = !initialLoadDone && loading && enigmas.length === 0 && !error;
    const showEmptyState = !loading && !error && enigmas.length === 0;
  if (showInitialSkeleton) {
    return (
    <div className="rounded-lg border border-dashed p-10 text-center text-sm text-muted-foreground">
Chargement des enigmes…</div>
    );
  } 
  return (
        ...
        <Table className="m-auto">
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-left">Numéro</TableHead>
