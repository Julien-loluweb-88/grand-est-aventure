...
export type GlobalBadgeListRow = {
  id: string;
  slug: string;
  title: string;
...
export async function listGlobalBadgesForAdmin(): Promise<GlobalBadgeListRow[] | null> {
  ...
  const rows = await prisma.badgeDefinition.findMany({
    where: {
      kind: { in: [...ADMIN_GLOBAL_BADGE_KIND_VALUES] },
      adventureId: null,
    },
...
export async function getGlobalBadgeForAdminEdit(
  id: string
): Promise<
  | {
      ok: true;
  ...
  | { ok: false; reason: "auth" | "missing" | "not_global" }
> {
  const actor = await getAdminActorForAuthorization();
  ...
  if (!(ADMIN_GLOBAL_BADGE_KIND_VALUES as readonly BadgeDefinitionKind[]).includes(row.kind)) {
