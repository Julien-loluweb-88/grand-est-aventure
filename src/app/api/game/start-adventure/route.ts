...
/**
 * Ouvre une session de jeu **avant** toute validation d’énigme (bouton « Commencer »).
 * Idempotent : si une session `IN_PROGRESS` existe déjà, ne crée pas de doublon.
 * Corps : `{ adventureId, userId }` — même session que `validate-enigma`.
 * Refusé si l’aventure n’a pas au moins une énigme **ou** un trésor.
 */
export async function POST(request: NextRequest) {
  ...
  const rl = checkRateLimit(
    `start-adventure:${ip}:${session.user.id}`,
    MAX_PER_WINDOW,
    WINDOW_MS
  );
  if (!rl.ok) {
  ...
  const hasEnigmas = adventure._count.enigmas > 0;
  const hasTreasure = adventure.treasure != null;
  if (!hasEnigmas && !hasTreasure) {
    return NextResponse.json(
      {
        error:
          "Cette aventure n’a ni énigme ni trésor : impossible de démarrer une partie.",
