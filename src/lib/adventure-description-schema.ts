...
/** Message affiché après validation du code coffre (trésor). */
export const treasureFinishMessageCreateZod = tiptapDoc.superRefine((doc, ctx) => {
  const len = tiptapJsonPlainTextLength(doc);
  if (len < 3) {
    ctx.addIssue({
      code: "custom",
  ...
  if (len > TREASURE_FINISH_MESSAGE_PLAIN_MAX_CHARS) {
    ctx.addIssue({
      code: "custom",
      message: `Le message ne doit pas dépasser ${TREASURE_FINISH_MESSAGE_PLAIN_MAX_CHARS} caractères.`,
    });
  }
});

export const treasureFinishMessageEditZod = tiptapDoc.superRefine((doc, ctx) => {
  const len = tiptapJsonPlainTextLength(doc);
  if (len < 1) {
    ctx.addIssue({
      code: "custom",
