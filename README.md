...
- **Menu latéral** : entrées séparées **Aventures**, **Villes** (référentiel), **Avatars** (compagnons 3D / vignettes / `.glb` hébergé) et **Publicités** (encarts & offres partenaires).
...
| Chemin | Fonctionnalité |
|--------|----------------|
| `/admin-game/dashboard/avatars` | Liste : slug, actif, présence vignette / modèle, nombre de joueurs ayant choisi cet avatar |
| Création / édition | Nom, ordre, actif ; **téléversement** vignette (JPEG/PNG/WebP) et modèle **`.glb`** sous `uploads/avatars/{id}/` ; API jeu expose `thumbnailUrl` et `modelUrl` (`GET /api/game/avatars`) |
| Fiche **utilisateur** | Bloc « Avatar compagnon » : préférence enregistrée et lien vers la fiche avatar |
...
| POST | `/api/game/validate-finish` | Finalisation **sans trésor** (après dernière énigme) : badges, `UserAdventures` |
| POST | `/api/game/validate-treasure` | Carte puis coffre ; fin de parcours, badges, stock physique |
| GET | `/api/game/adventure-partner-lots` | Roue partenaires : `?adventureId=` — état (`legalNotice`, `wheel`, segments, gain avec `validUntil` / `redeemed`) |
...
flowchart TB
  subgraph mobile["App mobile"]
    ...
    G --> S0["POST start-adventure — optionnel"]
    S0 --> H{Énigmes restantes ?}
    H -->|Oui| I["POST validate-enigma"]
    I --> H
    H -->|Non| T{Trésor ?}
    T -->|Oui| J["POST validate-treasure — carte"]
    J --> K["POST validate-treasure — coffre"]
    K --> L["Succès, badges…"]
    T -->|Non| V["POST validate-finish"]
    V --> L
    L --> W0["GET adventure-partner-lots — optionnel"]
    W0 --> W1{Roue ?}
    W1 -->|ready| W2["POST adventure-partner-lots/spin"]
    W1 -->|done / none| W3["—"]
    W2 --> W4["POST adventure-partner-lots/redeem — magasin, optionnel"]
    W3 --> W4
    W4 --> M["POST adventure-review — optionnel"]
...
### Parcours (énigmes & fin)

- [ ] **`GET /api/game/adventures/{id}`** : lire `enigmas[].multiSelect`, `enigmas[].choice`, `treasure` (présence / absence) pour l’UI et le flux suivant.  
- [ ] **`POST /api/game/start-adventure`** au « Commencer » (recommandé) : `sessionCreated` / `alreadyInProgress` ; **400** `EMPTY_ADVENTURE` si ni énigme ni trésor.  
- [ ] **`POST /api/game/validate-enigma`** : si `multiSelect` → `submissions` (string[]) ; sinon → `submission` (string). Gérer **400** (`WRONG_ANSWER`, `SUBMISSIONS_ARRAY_REQUIRED`, `SUBMISSION_STRING_REQUIRED`, `ORDER`).  
- [ ] **`POST /api/game/validate-finish`** : uniquement si **pas** de trésor, après la dernière énigme validée ; **400** `TREASURE_REQUIRED` ou `EMPTY_ADVENTURE`.  
- [ ] **`POST /api/game/validate-treasure`** : carte puis coffre si trésor présent (voir OpenAPI).

### Roue partenaires (fin d’aventure)

- [ ] **`GET /api/game/adventure-partner-lots?adventureId=`** : après victoire ; afficher `legalNotice` ; si `wheel === "ready"`, proposer la roue ; si `"done"`, afficher `won` (`validUntil`, `redeemed`, `winId`).  
- [ ] **`POST /api/game/adventure-partner-lots/spin`** : uniquement si `wheel === "ready"` ; corps `adventureId`, `userId` ; gérer **400** (`NO_PARTNER_LOTS`, `ADVENTURE_NOT_FINISHED`) et **503** `SPIN_RETRY`.  
- [ ] **`POST /api/game/adventure-partner-lots/redeem`** : confirmation en magasin ; **200** avec `alreadyRedeemed: true` si déjà utilisé ; **400** `NO_WIN` si pas de gain.
...
- **Authentification** : **session Better Auth** (cookies, comme `POST /api/game/start-adventure` / `validate-enigma`).
...
- **[Better Auth + Expo](docs/expo-better-auth.md)** — session cookies pour les routes jeu (`start-adventure`, `validate-enigma`, `validate-finish`, `validate-treasure`, `claim-discovery`, etc.).  
- **[Flux API et logique de jeu](docs/flux-api-et-jeu.md)** — schéma des routes, auth, parcours, roue partenaires, lots, cron.  
- **[Checklist intégration app mobile](docs/integration-app-mobile-checklist.md)** — phases, cases à cocher, ordre de mise en œuvre (Expo).  
